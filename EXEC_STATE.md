...
| S-11 | Cierre de S-08 y cambio de formato de seis gráficos, elegido por el usuario sobre la lámina de propuestas (`Tablero Casa Óga - Formatos.dc.html`): D2 → apilada al 100 % con trama · D3 → barras divergentes de sobre/subexposición · D5a y D5b → par de composiciones al 100 % con conectores · M3 → barra única al 100 % · M2a → tres barras de conversión al 100 %, que es lo que cierra S-08 sin tocar el dato (cada barra va sobre su propia base, así que no hay escala lineal que aplaste la última). Dos primitivas nuevas en `graficos.jsx` (`BarrasApiladas100`, `BarrasDivergentes`) y `Embudo` se borra: no quedaba nadie llamándola. El título propuesto para M2a en la lámina ("ningún paso retiene más de un tercio") se descartó por falso: el primer paso retiene 35,1 % | app | usuario 26/08 | cerrado |
