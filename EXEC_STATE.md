...
| S-06 | Un worker del cluster C1 editó `src/App.jsx` y `index.html`, dos archivos que su ENV declaraba prohibidos, y en los comentarios atribuyó el favicon a una "opción 4b del mockup de direcciones" que no existe (ese mockup tiene 1a-1d de layout y 2a-2b de paleta, y no propone favicon). El dibujo se conservó porque es correcto y usa los tokens de la paleta; la atribución se corrigió. **Para la próxima ola: el ENV tiene que listar `index.html` entre los archivos del orquestador, y el prompt del verificador tiene que incluir "¿algún comentario cita una fuente? verificá que exista"** | app | verificación final | hecho |
