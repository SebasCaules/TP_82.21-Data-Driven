...
3. **Un par de números grandes** (`.ban-par`) arriba a la izquierda cuando la decisión mueve
   ...
   `D2.decisiones` (campos `decision`, `justificacion`), con el id `DC-nn` visible.
...
|---|---|---|---|---|---|
| V01 | `V01_QueDecidimos.jsx` | «15 decisiones sobre 13 archivos: 11 cambian el dato, 2 se declaran, 2 esperan al negocio» (conteos de `vistas.V01.por_estado`) | ¿qué se decidió y con qué alcance? | tabla de 15 filas: id, archivo, hallazgo, decisión (una frase), estado como pastilla `Semaforo` (aplicada = meta, declarada = cerca, pendiente = fuera) y la vista donde se ve | `decisiones`, `vistas.V01` |
...
| V03 | `V03_Cobertura.jsx` | «Cuatro meses de 2025 tienen menos del 60 % de las operaciones del año anterior» | ¿qué meses no se pueden leer? | `Linea` de la razón mes / mismo mes del año anterior, 2023-01 a 2025-12, con banda del umbral 0,60 y los meses marcados; par de números: riesgo al 31/12/2025 y al 31/08/2025 | `vistas.V03` |
...
| V05 | `V05_Devoluciones.jsx` | «Las 613 devoluciones ya estaban en las ventas: sumarlas dos veces inflaba 1,2 % las unidades» | ¿cómo se cuentan las devoluciones? | `Linea` doble: devoluciones por mes según la fecha de la fila negativa y según `fecha_devolucion`; `BarrasH` de motivos | `vistas.V05` |
| V06 | `V06_Envios.jsx` | «200 envíos repetidos y 399 bajas de 2026 fuera: 568 → N contactables de la lista de 800» | ¿a quién se le puede escribir? | `BarraTramos` de la lista de 800: contactables / sin consentimiento / con baja; par de números de envíos antes/después y embudo | `vistas.V06` |
| V07 | `V07_Campanias.jsx` | «Con el join arreglado, la conversión por oferta se mide sobre 23.529 envíos y no sobre 22.614» | ¿qué oferta convierte? | `PuntosIC` antes/después por tipo de oferta; tarjeta con los dos casos CAMP004 y CAMP034 y la fila elegida; tarjeta Gold (5.066 envíos, 3 socios) | `vistas.V07` |
| V08 | `V08_Precios.jsx` | «El precio unitario bajó 7,6 % en pesos corrientes mientras el IPC multiplicó por 13» | ¿por qué no se deflacta? | dos `Linea` en paneles separados (mediana anual; IPC acumulado base 2022 = 1), nunca eje doble; `BarrasH` de variación por categoría | `vistas.V08` |
| V09 | `V09_NPS.jsx` | «El 28,8 % de las filas de soporte trae NPS sin reclamos ni consultas: se marca, no se borra» | ¿cuánto vale el NPS? | `Linea` doble del NPS anual (con todo / solo con interacción); `BarrasH` de reclamos por cliente-mes por año; tarjeta riesgo vs soporte | `vistas.V09` |
| V10 | `V10_CifraCentral.jsx` | «Tras las decisiones, el riesgo es 50,4 % de los elegibles y la exposición ARS 96,4 M/año» | ¿cambió la cifra del directorio? | tres pares grandes (E1 → después → sensibilidad 31/08/2025); `BarrasH` de cambios por decisión (Δ pp, Δ M); pie: exposición no es recupero; en revisión | `vistas.V10` |
